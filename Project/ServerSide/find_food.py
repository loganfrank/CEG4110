...
#	image_path = args.image_path
	...
	confidence = abs(abs(scores[0][0]) - abs(scores[0][1]))
	returnArray = []
	if np.argmax(scores) == 1:
	    print "No food here... :( "
	    confidencePercentage = (confidence / scores[0][1]) * 100
	    print 'Confidence in response: {} %'.format(confidencePercentage)
	    returnArray.append(1)
	    returnArray.append(confidencePercentage)
	else:
	    print "Oh yes... I see food! :D"
	    confidencePercentage = (confidence / scores[0][0]) * 100
	    print 'Confidence in response: {} %'.format(confidencePercentage)
